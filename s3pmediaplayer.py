...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, *args, **kwargs):
        ...
        # Connecting control buttons/slides for media player.
        self.playButton.pressed.connect(self.player.play)
        self.pauseButton.pressed.connect(self.player.pause)
        self.stopButton.pressed.connect(self.player.stop)
        self.volumeSlider.valueChanged.connect(self.player.setVolume)
        self.download.pressed.connect(self.ytdownload)
    ...
    def ytdownload(self):
        ...
        self.video = self.video.streams.get_highest_resolution()
        i = self.video.download(self.savepath)
        self.playlist.addMedia(QMediaContent(QUrl.fromLocalFile(i)))
        self.model.layoutChanged.emit()

    def open_file(self):
        filter = " All files (*.*);; mp3 Audio (*.mp3);; mp4 Video (*.mp4);; Movie files (*.mov)"
        path, _ = QFileDialog.getOpenFileNames(self, "Open file", "",filter )
        for i in path:
            if i:
                self.playlist.addMedia(
                    QMediaContent(
...
if __name__ == '__main__':
    app = QApplication([])
    app.setApplicationName("S3P Media Player")
    #app.setStyle("Fusion")
    app.setStyle("Fusion")
    palette = QPalette()
